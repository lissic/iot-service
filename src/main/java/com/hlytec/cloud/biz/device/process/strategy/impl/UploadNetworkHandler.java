 ...
 */
@Slf4j
@Component("/ToSer/RYX/1_SetAppIpPort")
public class UploadNetworkHandler implements ProcessMqttMsgStrategy {
    @Autowired
    ...
    public void processMqttMsg(String message) {
        log.info("upload message from network,{}", message);
        // 推送结果至前端
        ...
        DeviceLog deviceLog = logService.findOneWithResultNull(DeviceLog.builder().cardId(cardId).operationType(1)
